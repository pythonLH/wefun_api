...
@ddt
class add_article(unittest.TestCase):
    quanzi_case = DoExcel(QuanZi_case_dir).get_case('圈子信息块加入或移除')
    ...
    def setUp(self):
        ...
        article_status = mysql.fetch_one(sql)['status'] # 事先查出：第一条用例时"9db154e2be2e450488bb8dd14c767f4b"在第四个圈子中的状态,应该是为0
                                                                  #第二条用例时"9db154e2be2e450488bb8dd14c767f4b"在第四个圈子中的状态,应该是为1
    ...
    def test_01(self,case):
        ...
        if result['success'] == True and result['message'] == '成功':
            ...
            mysql_expected = mysql.fetch_one("select * from message.message_circle_link where circle_uuid = '555c54bb67a546569969e8c5d8d0c538' and message_uuid = '9db154e2be2e450488bb8dd14c767f4b';")
            # 第一条用例调用后status是1；第二条用例调用后然后变成0
            self.assertNotEqual(article_status,mysql_expected['status']) # 接口调用前的状态，跟接口调用后的状态，bu一致PASS
