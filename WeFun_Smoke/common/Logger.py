...
class My_Logger:
    ...
    def console(self,level,message):
        ...
        # info信息输出到指定文件
        info_logger = logging.FileHandler(filename='D:\WeFun Smoke\logger\info_logger\info.log',encoding='utf-8')
        info_logger.setLevel('INFO')
        info_logger.setFormatter(self.formatter)
        self.my_logger.addHandler(info_logger)

        # error信息输出到指定文件
        error_logger = logging.FileHandler(filename='D:\WeFun Smoke\logger\error_logger\error.log',encoding='utf-8')
